...
def write_equi_params(mof, out_dir, mol_name=None):
    ...
    if os.path.exists(f'{out_dir}/emin_{mof}.param'):
        os.remove(f'{out_dir}/emin_{mof}.param')
    
    with open(f'{out_dir}/emin_{mof}.param', 'a') as f:
...
def write_equi_input(mof, out_dir, nvt, npt, mol_name=None, n_cation=None, equi_time=100000, temp=300, pressure=0.0):
    ## read the input file
    mof_styles = read_styles(f'{out_dir}/in.{mof}')
    mof_param = read_param(f'{out_dir}/data.{mof}')
    ...
    modify_mof_data(f'{out_dir}/data.{mof}', total_atm_num, total_b_num, total_a_num, total_d_num, total_im_num, 'new')
    
    random_seed = random.randint(1, 1000000)

    ## initilization section
    with open(f'{out_dir}/in.emin_{mof}', 'w') as f:
        ...
        if mol_name is not None:
            ...
            f.write(output_template_no_mol.format(mof = mof))
