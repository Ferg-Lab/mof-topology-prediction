...
def cif2lmp(mol_name, forcefield, out_dir):
    
    mol_path = os.getcwd()
    mol_cif = os.path.join(mol_path, f'small_molecule/{mol_name}.cif')
    ...
    os.system(command)
    os.system(f'mv data.{mol_name} in.{mol_name} ./{out_dir}')

def create_mol_file(mol_name, forcefield, out_dir):
    cif2lmp(mol_name, forcefield, out_dir)
    
    in_mol = os.path.join(out_dir, f"in.{mol_name}")
    data_mol = os.path.join(out_dir, f"data.{mol_name}")
    
    mol_data = read_data(data_mol)
    if mol_data is None:
        logging.error(f"Failed to create molecule file for {mol_name}.")
        return

    styles = read_styles(in_mol)

    mol_file = os.path.join(out_dir, f"{mol_name}.txt")
