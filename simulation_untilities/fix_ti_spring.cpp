...
void FixTISpring::post_force(int /*vflag*/)
{
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      ...
      f[i][0] = f[i][0] + lambda * (-k*dx); /* modified so that interatomic interactions are not scaled */
      f[i][1] = f[i][1] + lambda * (-k*dy); /* modified so that interatomic interactions are not scaled */
      f[i][2] = f[i][2] + lambda * (-k*dz); /* modified so that interatomic interactions are not scaled */
