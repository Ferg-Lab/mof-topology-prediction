...
def build_mof(node_name, linker_name, topo_list, forcefield, replication, outdir):
    ...
    for topo in topo_list:
        ...
        try:
            ...
            mof.write_cif(f'./{outdir}/{topo}-{node_name}-{linker_name}.cif')
            if replication is not None:
                command = f'lammps-interface ./{outdir}/{topo}-{node_name}-{linker_name}.cif -ff {forcefield} --replication {replication} > tmp; rm tmp'
                os.system(command)
            else:
                command = f'lammps-interface ./{outdir}/{topo}-{node_name}-{linker_name}.cif -ff {forcefield} > tmp; rm tmp'
                os.system(command)
            os.system(f'mv data.{topo}-{node_name}-{linker_name} in.{topo}-{node_name}-{linker_name} ./{outdir}')
