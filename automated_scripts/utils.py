...
def read_styles(mol_name):
    infile = f'in.{mol_name}'
    styles = {}
    try:
        ...
        logging.error(f"{infile} not found.")
    return styles
...
def read_data(mol_name):
    if os.path.exists(f'data.{mol_name}'):
        data_file = lammpsio.DataFile(f'data.{mol_name}', atom_style='full')
...
def read_param(mol_name):
    data_file = f'data.{mol_name}'
...
def modify_mof_data(mof, total_atm_num, total_b_num, total_a_num, total_d_num, total_im_num, job):
    ...
    with open(f'data.{mof}_{job}', 'w') as f:
